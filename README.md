<div align="center">

<!-- Header - Cylinder style, more modern than wave -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=cylinder&color=0:0f2027,50:203a43,100:2c5364&height=160&section=header&text=Mohammed%20Saqib&fontSize=42&fontColor=e0f7fa&fontAlignY=55&desc=Software%20Tester%20%7C%20QA%20Engineer%20%7C%20Automation%20Learner&descSize=16&descAlignY=78&descColor=80cbc4"/>

<!-- Typing SVG -->
[![Typing SVG](https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=18&pause=1000&color=4DD0E1&center=true&vCenter=true&width=600&lines=Final-year+MCA+%7C+Targeting+QA+%2F+SDET+Roles;Python+%7C+Selenium+%7C+Manual+%26+Automation+Testing;Real+tests.+Real+bugs.+Real+documentation.)](https://git.io/typing-svg)

<br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Mohammed%20Saqib-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/mohammed-saqib-b2771836b/)
[![GitHub](https://img.shields.io/badge/GitHub-saqib777-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/saqib777)
[![Email](https://img.shields.io/badge/Email-mohammedsaqib532%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:mohammedsaqib532@gmail.com)
[![Location](https://img.shields.io/badge/Location-Bengaluru%2C%20India-34A853?style=for-the-badge&logo=google-maps&logoColor=white)](#)

</div>

---

## About Me

I am a final-year **MCA student** from Karnataka, actively transitioning into a **Software Tester / QA Engineer** role with a long-term goal of becoming an **SDET**.

My approach is grounded in fundamentals — disciplined test case design, structured documentation, and hands-on automation practice. I build real projects, write real test cases, and document real bugs. No filler, no shortcuts.

- **Primary Language:** Python
- **Current Focus:** Selenium WebDriver, UI automation, pytest-based test frameworks
- **Actively applying** to QA / Software Testing roles in Bengaluru
- **Learning publicly** — every project is documented and traceable

---

## Tech Stack & Tools

**Testing & QA**

![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=flat-square&logo=selenium&logoColor=white)
![Katalon Studio](https://img.shields.io/badge/Katalon%20Studio-1D3557?style=flat-square&logo=katalon&logoColor=white)
![pytest](https://img.shields.io/badge/pytest-0A9EDC?style=flat-square&logo=pytest&logoColor=white)
![Manual Testing](https://img.shields.io/badge/Manual%20Testing-444444?style=flat-square)
![Smoke Testing](https://img.shields.io/badge/Smoke%20Testing-555555?style=flat-square)
![Regression Testing](https://img.shields.io/badge/Regression%20Testing-555555?style=flat-square)
![Performance Testing](https://img.shields.io/badge/Performance%20Testing-2C3E50?style=flat-square)
![Exploratory Testing](https://img.shields.io/badge/Exploratory%20Testing-555555?style=flat-square)
![Test Case Design](https://img.shields.io/badge/Test%20Case%20Design-37474F?style=flat-square)
![Edge Case Analysis](https://img.shields.io/badge/Edge%20Case%20Analysis-37474F?style=flat-square)
![Flaky Test Detection](https://img.shields.io/badge/Flaky%20Test%20Detection-6D4C41?style=flat-square)
![Bug Reporting](https://img.shields.io/badge/Bug%20Reporting-B71C1C?style=flat-square)
![Test Documentation](https://img.shields.io/badge/Test%20Documentation-455A64?style=flat-square)

**Languages**

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=openjdk&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![C](https://img.shields.io/badge/C-00599C?style=flat-square&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-00427E?style=flat-square&logo=cplusplus&logoColor=white)
![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white)
![HTML](https://img.shields.io/badge/HTML-E34F26?style=flat-square&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS-1572B6?style=flat-square&logo=css3&logoColor=white)

**Tools & Platforms**

![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=flat-square&logo=visual-studio-code&logoColor=white)
![PyCharm](https://img.shields.io/badge/PyCharm-21D789?style=flat-square&logo=pycharm&logoColor=black)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=flat-square&logo=firebase&logoColor=black)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat-square&logo=linux&logoColor=black)
![Tally ERP 9](https://img.shields.io/badge/Tally%20ERP%209-1A237E?style=flat-square)

**3D & Design**

![Blender](https://img.shields.io/badge/Blender-F5792A?style=flat-square&logo=blender&logoColor=white)
![Unity](https://img.shields.io/badge/Unity-100000?style=flat-square&logo=unity&logoColor=white)
![Womp](https://img.shields.io/badge/Womp%203D-2D2D2D?style=flat-square)
![3D Modeling](https://img.shields.io/badge/3D%20Modeling-37474F?style=flat-square)

---

## Featured Projects

<table>
<tr>
<td width="50%" valign="top">

### Software Testing Projects
**[`saqib777/Software_Testing_Projects`](https://github.com/saqib777/Software_Testing_Projects)**

End-to-end manual and automation testing suite covering real web applications. Includes structured test plans, detailed test cases, execution logs, and traceability matrices.

`Selenium` `pytest` `Python` `Test Plans` `Katalon`

</td>
<td width="50%" valign="top">

### Ministry of Testing
**[`saqib777/Ministry_of_testing`](https://github.com/saqib777/Ministry_of_testing)**

Structured test scenarios and exploratory testing sessions built around real-world applications. Covers edge cases, negative flows, and session-based test notes.

`Exploratory Testing` `Test Design` `Edge Cases`

</td>
</tr>
<tr>
<td width="50%" valign="top">

### Bug Reports Portfolio
**[`saqib777/Bug-Reports-Portfolio`](https://github.com/saqib777/Bug-Reports-Portfolio)**

Professional-grade bug reports from real test executions. Each report includes severity classification, reproduction steps, expected vs actual results, and environment details.

`Bug Reporting` `Defect Lifecycle` `Documentation`

</td>
<td width="50%" valign="top">

### DSA Journal
**[`saqib777/dsa-journal`](https://github.com/saqib777/dsa-journal)**

Python DSA solutions tracked with pytest test cases, algorithm annotations, and time/space complexity notes. Built to strengthen problem-solving fundamentals while maintaining a contribution streak.

`Python` `pytest` `DSA` `Algorithms`

</td>
</tr>
</table>

> Browse all repositories: [github.com/saqib777?tab=repositories](https://github.com/saqib777?tab=repositories)

---

## What I'm Currently Working On

<table>
<tr>
<th align="left">Area</th>
<th align="left">Detail</th>
<th align="left">Status</th>
</tr>
<tr>
<td><strong>Selenium WebDriver</strong></td>
<td>UI automation with Python — locators, waits, page object model</td>
<td><code>Active</code></td>
</tr>
<tr>
<td><strong>pytest Framework</strong></td>
<td>Test structuring, fixtures, parametrize, assertions</td>
<td><code>Active</code></td>
</tr>
<tr>
<td><strong>DSA Journal</strong></td>
<td>Daily Python solutions with test cases and complexity notes</td>
<td><code>Streak Maintained</code></td>
</tr>
<tr>
<td><strong>Bug Report Portfolio</strong></td>
<td>Real bug findings from live applications with SDET-style RCA</td>
<td><code>Building</code></td>
</tr>
<tr>
<td><strong>QA Job Applications</strong></td>
<td>Active sprint targeting Bengaluru-based Software Testing roles</td>
<td><code>In Progress</code></td>
</tr>
</table>

---

## GitHub Stats

<div align="center">

<!-- Fix 2: Switched to github-readme-stats for top languages — updates faster than profile-summary-cards -->
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=saqib777&layout=compact&theme=github_dark&hide_border=true&langs_count=10&card_width=900" width="100%"/>

<br/><br/>

<!-- Fix 1: Switched to streak-stats.demolab.com — more reliable daily updates than herokuapp -->
<img src="https://streak-stats.demolab.com/?user=saqib777&theme=dark&hide_border=true&background=0d1117&stroke=4DD0E1&ring=4DD0E1&fire=ff6b6b&currStreakLabel=4DD0E1" />

</div>

---

## Writing & Knowledge Sharing

I document my testing journey and share insights on LinkedIn — covering test design, real bug findings, automation experiments, and career progress.

**[Follow along on LinkedIn](https://www.linkedin.com/in/mohammed-saqib-b2771836b/)**

---

<div align="center">

*"Consistency and discipline matter more than speed. I focus on showing real work, not just titles."*

<br/>

<!-- Footer matching cylinder header style -->
<img width="100%" src="https://capsule-render.vercel.app/api?type=cylinder&color=0:2c5364,50:203a43,100:0f2027&height=100&section=footer"/>

</div>
